...
# ------------- self-update capability (sudoers + trigger script) -------------
# Lets the running app (unprivileged) kick off self_update.sh (root) via
# `sudo -n`, without granting it broad sudo rights — the NOPASSWD rule is
# scoped to this exact, freshly-generated script path only. Regenerated
# on every install/update since it bakes in the resolved INSTALL_DIR.
info "Kendi kendini güncelleme yetkisi kuruluyor..."
cat > "${INSTALL_DIR}/scripts/trigger_update.sh" <<TRIGGER
#!/usr/bin/env bash
# Generated by install.sh/update.sh — regenerated on every run, don't
# hand-edit. systemd-run detaches self_update.sh into its own cgroup so
# it survives rtcview.service being stopped mid-update.
set -euo pipefail
exec systemd-run --unit=rtcview-selfupdate \\
  --description="RtcView self-update" \\
  --collect \\
  ${INSTALL_DIR}/scripts/self_update.sh
TRIGGER
chmod 0755 "${INSTALL_DIR}/scripts/trigger_update.sh"
chmod 0755 "${INSTALL_DIR}/scripts/self_update.sh" 2>/dev/null || true

SUDOERS_FILE="/etc/sudoers.d/rtcview-selfupdate"
cat > "${SUDOERS_FILE}.new" <<EOF
${SERVICE_USER} ALL=(root) NOPASSWD: ${INSTALL_DIR}/scripts/trigger_update.sh
EOF
if visudo -cf "${SUDOERS_FILE}.new" >/dev/null 2>&1; then
  chmod 0440 "${SUDOERS_FILE}.new"
  mv "${SUDOERS_FILE}.new" "$SUDOERS_FILE"
  info "sudoers kuralı eklendi: $SUDOERS_FILE"
else
  rm -f "${SUDOERS_FILE}.new"
  warn "sudoers kuralı doğrulanamadı — Ayarlar'daki 'Şimdi Güncelle' düğmesi çalışmayacak."
fi
