...
3. `app/VERSION` dosyası aynı sürüm numarasına güncellenir — bu, uygulamanın tek gerçek kaynağı (`/api/status`'un `version` alanı ve Ayarlar → Sistem → Güncelleme'deki sürüm satırı buradan okur). CHANGELOG'u güncelleyip bu dosyayı unutmak, sürümün sessizce eskimiş görünmesine yol açar.
4. Mümkünse commit sonrası `git tag -a vX.Y.Z -m "..."` ile etiketlenir ve `git push origin vX.Y.Z` denenir. Bu oturumun git kimlik bilgileri yalnızca tanımlı branch'e push izinlidir — tag push 403 ile reddedilirse (ve GitHub MCP araçlarında tag/release oluşturan bir araç yoksa) kullanıcıya bunu bildir, `CHANGELOG.md` kaydını yine de commit'e dahil et; tag'i kullanıcı isterse kendisi push eder veya GitHub arayüzünden Release olarak oluşturur.
